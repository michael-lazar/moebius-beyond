...
function half_block_ellipse_overlay(sx, sy, dx, dy, col) {
    ...
    overlay.update(
        x * font.width,
        Math.floor((y * font.height) / 2),
        (radius_x * 2 + 1) * font.width,
        Math.floor(((radius_y * 2 + 1) * font.height) / 2)
    );
    ...
    for (const coord of coords)
        overlay.fill_rect(
            coord.x * font.width,
            Math.floor((coord.y * font.height) / 2),
            font.width,
...
function draw_blink_ellipse(sx, sy, dx, dy, unblink) {
    ...
    for (const coord of coords) {
        ...
        if (
            block &&
            ((!unblink && block.bg < 8) || (unblink && block.bg >= 8)) &&
            block.code != 0 &&
            block.code != 32 &&
            block.code != 255
        )
